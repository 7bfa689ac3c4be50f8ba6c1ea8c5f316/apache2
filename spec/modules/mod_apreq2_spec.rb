...
describe 'ga-apache2::mod_apreq2' do
    ...
    versions.each do |version|
      context "on #{platform.capitalize} #{version}" do
        ...
        if %w(suse).include?(platform)
          it 'installs package ga-apache2-mod_apreq2' do
            expect(chef_run).to install_package('ga-apache2-mod_apreq2')
            expect(chef_run).to_not install_package('not_apache2-mod_apreq2')
          end
          let(:package) { chef_run.package('ga-apache2-mod_apreq2') }
          it 'triggers a notification by ga-apache2-mod_apreq2 package install to execute[generate-module-list]' do
            expect(package).to notify('execute[generate-module-list]').to(:run)
            expect(package).to_not notify('execute[generate-module-list]').to(:nothing)
          end
        end
        ...
        it_should_behave_like 'an ga-apache2 module', 'apreq', false
      end
    end
  end
end
